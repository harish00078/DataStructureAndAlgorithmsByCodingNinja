...
public class RemoveLeafNOdes {
    ...
    public static CreateBinaryTreeNode<Integer> removeLeaves(CreateBinaryTreeNode<Integer> root){

        if(root == null){ // here we firslty have to check that: if root is null
            return null; // then we simply return null:
        }

        if(root.left == null && root.right == null){ // here we check if roots left or roots rights values are null:
            // it means root does not have any leaf:
            return null; // so we can simply return  null:
        }

        root.left = removeLeaves(root.left); // here we use recusrion for removing the leaf values from the roots left nodes:
        // and after removing the leaf  values: we convert  that leaf is equal to the null:
        // because if we did not convert that values equal to nulll:
        //  then our system will we gave us a output as a null:
        root.right = removeLeaves(root.right); // same as a roots left nodes:
        return root; // and after removing the nodes we simply return that root value:
    }
    ...
    // here we create function: for mirror the image of   Binary Tree structure:
    public static void mirrorBinaryTree(CreateBinaryTreeNode<Integer> root){
		//Your code goes here
        if(root == null){ // here we have to check: if root is null
            return ; // then we simply return nothing:
        }
        if(root.left==null && root.right==null){ // here we also have to check: if roots left or roots right node is null:
            return; // then we simply return nothing:

        }
     
        mirrorBinaryTree(root.left);  //  here we use recursion: on the roots.left nodes: so we can create the mirror image of the Binary Tree Structure:
        mirrorBinaryTree(root.right); //  here we use recursion: on the roots.right nodes: so we can create the mirror image of the Binary Tree Structure:

        // after creating the mirror image of the original tree sturcture:
        // we have to swipe its values: so we can create the excet image of the original Binary Tree Structure:
        CreateBinaryTreeNode<Integer> temp=root.left;
    ...
    public static void main(String[] args) {
        ...
        // here we are using the remove leaves function: so we can remove the leaves from the Binary Tree Structure:
        // CreateBinaryTreeNode<Integer> newRoot =  removeLeaves(root); 
        // printTreeDetailed(newRoot);

        // here we are using the mirror image function: so we can create the mirror image of the original binary Tree Structure:
        System.out.println("mirror image of the original Tree Structure");
        mirrorBinaryTree(root); // here we create the mirror image of the original binary Tree Structure:
        printTreeDetailed(root); // and here we print that mirror image:
